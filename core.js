...
/* core.js - Jewels-Ai: Master Engine (v13.0 - Performance Optimized) */
...
let lastFaceCheck = 0;
let lastHandCheck = 0;
const TRACKING_INTERVAL = 100; // Check AI every 100ms (Prevent lag)
...
/* --- 2. AI CONCIERGE (HIDDEN BUT FUNCTIONAL) --- */
const concierge = {
    ...
    init: function() {
        ...
        this.setVoice();
        // Force hide via JS to ensure it stays hidden
    ...
    toggle: function() {
        if (!this.hasStarted) {
            ...
            this.speak("Vanakkam! I am Sai Pallavi, your personal jewelry stylist.");
...
function toggleStacking() {
    ...
    if (window.JewelsState.stackingEnabled) {
        ...
        concierge.speak("Stacking enabled.");
    } else {
        ...
        concierge.speak("Single mode active.");
    }
}
...
function initVoiceControl() {
    ...
    recognition.onstart = () => { 
        ...
        if(voiceBtn) { 
            ...
            voiceBtn.style.boxShadow = "0 0 10px rgba(0,255,0,0.5)";
        } 
    };
    ...
    recognition.onerror = (e) => {
        if(e.error === 'not-allowed') {
            alert("Microphone blocked. Please allow permissions.");
            voiceEnabled = false;
        }
    };
    ...
    recognition.onend = () => { 
        ...
        if (voiceEnabled) setTimeout(() => { try { recognition.start(); } catch(e) {} }, 500); 
            ...
            voiceBtn.style.backgroundColor = "rgba(0,0,0,0.6)"; 
...
function processVoiceCommand(cmd) { 
    ...
    triggerVisualFeedback("🎤 " + cleanCmd);

    if (cleanCmd.includes('next') || cleanCmd.includes('change')) { changeProduct(1); return; } 
    if (cleanCmd.includes('back') || cleanCmd.includes('previous')) { changeProduct(-1); return; } 
...
/* --- 8. CAMERA & TRACKING (PERFORMANCE FIXED) --- */
async function startCameraFast(mode = 'user') {
    ...
    try {
        ...
        try {
            // Attempt 2: Low Quality Fallback (Better for "Stuck" video issues)
...
/* --- 9. RENDER LOOPS (OPTIMIZED) --- */
async function detectLoop() {
    if (videoElement.readyState >= 2 && !remoteVideo.srcObject) { 
        const now = Date.now();
        
        // Throttled Face Check
        if (now - lastFaceCheck > TRACKING_INTERVAL) {
             if (!isProcessingFace) { 
        ...
        // Throttled Hand Check
        if (now - lastHandCheck > TRACKING_INTERVAL) {
             if (!isProcessingHand) { 
        ...
        // Note: Canvas drawing now happens inside onResults but is smooth because calls are spaced out
    }
...
faceMesh.onResults((results) => {
  ...
  // ALWAYS DRAW THE VIDEO FIRST (Prevents stuck screen)
  ...
  if (!earringImg && !necklaceImg) { canvasCtx.restore(); return; }
...
hands.onResults((results) => {
  ...
  // Hand rendering is done separately to face to avoid double-draw. 
  // It only draws IF rings/bangles are active.
